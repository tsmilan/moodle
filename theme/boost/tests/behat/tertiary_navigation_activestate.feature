...
I need to see accurate checkmarks when navigating back and forward
  ...
  Scenario: Ensure checkmark is not visible if link is not navigated to in current browser
    Given I log in as "teacher1"
    When I am on "Course 1" course homepage
    And I navigate to "Grades" in current page administration
    Then dropdown item "Grader report" should be active
    When I click on "Grader report" "combobox"
    And I update the "value" data attribute of the "Scales" combobox item to "#"
    And I select "Scales" from the dropdown
    Then dropdown item "Scales" should not be active
    When I update the "value" data attribute of the "Export" combobox item to "#"
    And I click on "Grader report" "combobox"
    And I select "Export" from the dropdown
    Then dropdown item "Export" should not be active
