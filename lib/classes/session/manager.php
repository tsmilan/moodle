...
class manager {
    ...
    public static function get_performance_info() {
        ...
        if (!empty($CFG->debugsessionlock)) {
            ...
            if (!empty($sessionlock['held'])) {
                ...
                $sessionlockheldtext = "Session lock held: ".number_format($sessionlock['held'], 3)." secs";
                $info['txt'] .= $sessionlockheldtext;
                $info['html'] .= html_writer::div($sessionlockheldtext, "sessionlockstart");
            } else {
                ...
                $sessionlockopentext = "Session lock open: ".number_format($sessionlockheld, 3)." secs";
                $info['txt'] .= $sessionlockopentext;
                $info['html'] .= html_writer::div($sessionlockopentext, "sessionlockstart");
            }
     ...
     *
     * @param  array $recentsessionlocks Recent session locks array.
     * @param  array $sessionlock Session lock array
     * @return array Recent session locks array.
     */
