...
EOF;
     ...
     */
    public function test_readfile_accel_with_path_and_stored_file() {
        $this->resetAfterTest();
        ...
        $this->assertStringContainsString('Non-empty default output handler buffer detected while serving the file ' . 
            $filepath . '. Buffer contents (first 20 chararacters): test text', $output);
        $this->assertStringContainsString('Non-empty default output handler buffer detected while serving the file ' .
            $filename . '. Buffer contents (first 20 chararacters): test text', $output);
    }
}
